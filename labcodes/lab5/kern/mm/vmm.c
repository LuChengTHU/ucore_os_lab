...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
    ...
    */
    ...
    if (*ptep == 0) {
        struct Page* phy_page = pgdir_alloc_page(mm->pgdir, addr, perm);
        if (phy_page == NULL) {
            cprintf("Cannot allocate a page of phy address in do_pgfault!\n");
            goto failed;
    ...
    */
        if(swap_init_ok) {
            ...
            if (swap_in(mm, addr, &page) != 0) {
                cprintf("swap_in crashed in do_pgfault\n");
